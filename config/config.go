...
func Init() {
	...
	post, err := db.Prepare(`CREATE TABLE IF NOT EXISTS posts(id SERIAL PRIMARY KEY, thread TEXT, content TEXT, creator_id INTEGER, create_time datetime,   update_time datetime DEFAULT CURRENT_TIMESTAMP, image	BLOB NOT NULL, count_like INTEGER DEFAULT 0, count_dislike INTEGER DEFAULT 0, FOREIGN KEY(creator_id) REFERENCES users(id) ON DELETE CASCADE ) `)
	if err != nil {
		log.Println(err)
	}
	session, err := db.Prepare(`CREATE TABLE IF NOT EXISTS "session"("id" SERIAL PRIMARY KEY, "uuid"	TEXT, "user_id"	INTEGER UNIQUE, cookie_time datetime)`)
	if err != nil {
		log.Println(err)
	}
	user, err := db.Prepare(`CREATE TABLE IF NOT EXISTS "users"("id" SERIAL NOT NULL PRIMARY KEY, "full_name" TEXT NOT NULL, "email"	TEXT NOT NULL UNIQUE, "username" TEXT NOT NULL UNIQUE, "password" TEXT, "isAdmin" INTEGER DEFAULT 0, "age" INTEGER, "sex" TEXT, "created_time"	datetime, "last_seen" datetime, "city" TEXT, "image"	BLOB NOT NULL)`)
	if err != nil {
		log.Println(err)
	}
	...
	category, err := db.Prepare(`CREATE TABLE IF NOT EXISTS  category(id SERIAL PRIMARY KEY, name TEXT UNIQUE)`)
	if err != nil {
		log.Println(err)
	}
